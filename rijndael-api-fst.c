...
int rijndael_makeKey(rijndaelKeyInstance *key, BYTE direction, int keyLen, char *keyMaterial) {
...
int rijndael_cipherInit(rijndaelCipherInstance *cipher, BYTE mode, char *IV) {
	if ((mode == RIJNDAEL_MODE_ECB) || (mode == RIJNDAEL_MODE_CBC) || (mode == RIJNDAEL_MODE_CFB1)) {
		cipher->mode = mode;
	} else {
...
int rijndael_blockEncrypt(rijndaelCipherInstance *cipher, rijndaelKeyInstance *key,
 ...
 */
int rijndael_padEncrypt(rijndaelCipherInstance *cipher, rijndaelKeyInstance *key,
...
int rijndael_blockDecrypt(rijndaelCipherInstance *cipher, rijndaelKeyInstance *key,
		BYTE *input, int inputLen, BYTE *outBuffer) {
	int i, k, numBlocks;
	u_int8_t block[16], iv[4][4];
	...
	if (cipher == NULL ||
		key == NULL ||
		(cipher->mode != RIJNDAEL_MODE_CFB1 && key->direction == DIR_ENCRYPT)) {
		return BAD_CIPHER_STATE;
	}
	if (input == NULL || inputLen <= 0) {
		return 0; /* nothing to do */
...
int rijndael_padDecrypt(rijndaelCipherInstance *cipher, rijndaelKeyInstance *key,
		BYTE *input, int inputOctets, BYTE *outBuffer) {
	int i, numBlocks, padLen;
	u_int8_t block[16];
	u_int32_t iv[4];
